...
public sealed class NomenclaturaVialTool
{
    ...
    public void Run()
    {
        ...
        try
        {
            ...
            while (true)
            {
                var pointOptions = new PromptPointOptions("\nPunto de inserción (ESC o clic derecho para cancelar): ") { AllowNone = true };
                PromptPointResult pointResult = editor.GetPoint(pointOptions);
                if (pointResult.Status == PromptStatus.Cancel || pointResult.Status == PromptStatus.None) break;
                if (pointResult.Status != PromptStatus.OK) continue;
                Point3d position = pointResult.Value;
                string? tipo = ObtenerTipo(editor); if (tipo is null) break;
    ...
    {
        var options = new PromptKeywordOptions("\nTipo de vía [KR/CL] (ESC o clic derecho para cancelar): ") { AllowNone = true };
    ...
    {
        var options = new PromptKeywordOptions("\nTipo de superficie [T.N./PAV/ASF/ADO] (ESC o clic derecho para cancelar): ") { AllowNone = true };
