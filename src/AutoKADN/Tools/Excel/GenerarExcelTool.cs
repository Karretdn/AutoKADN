...
public sealed class GenerarExcelTool
{
    ...
    {
        new MaterialSpec("UNION", "1/2", "100003150"),
    ...
    public void Run()
    {
        ...
        try
        {
            ...
            foreach (UcKey uc in detectedUcs)
            {
                ...
                if (!materialQuantities.TryGetValue(uc, out quantities))
    ...
    // ESPIRAL aporta únicamente los materiales que no provienen de bloques físicos:
    // TUBERIA 3/4, UNION 3/4 y TEE 3/4.
    // VALVULA y SILLETA continúan viniendo de los bloques físicos de la capa Mat.
    // PE.EXT. y el diámetro de SILLETA son datos informativos y no se llevan como cantidad.
    // La clave de consolidación siempre conserva: descripción + diámetro + unidad + código,
    // agrupada además por UC = diámetro + terreno.
                ...
                foreach (ObjectId objectId in space)
                {
                    ...
                    for (int i = 0; i < values.Length; i++)
                    {
                        if (values[i].TypeCode == (int)DxfCode.ExtendedDataAsciiString &&
                            string.Equals(values[i].Value as string, SpiralXDataType, StringComparison.OrdinalIgnoreCase))
                        {
                            typeIndex = i;
                            break;
                        }
                    ...
                    string surface = values[index + 7].Value == null
                        ? string.Empty
                        : values[index + 7].Value.ToString().Trim();
                    surface = NormalizeSurface(surface);
                    if (string.IsNullOrWhiteSpace(surface)) continue;

                    UcKey uc = new UcKey("3/4", surface);

                    MaterialSpec pipeMaterial;
                    if (pipe > 0.0 && TryGetMaterialSpec("TUBERIA", "3/4", out pipeMaterial))
                        AddMaterialQuantity(result, uc, pipeMaterial, "ML", Math.Abs(pipe));

                    MaterialSpec unionMaterial;
                    if (unions > 0.0 && TryGetMaterialSpec("UNION", "3/4", out unionMaterial))
                        AddMaterialQuantity(result, uc, unionMaterial, "UND", Math.Abs(unions));

                    MaterialSpec teeMaterial;
                    if (tees > 0.0 && TryGetMaterialSpec("TEE", "3/4", out teeMaterial))
                        AddMaterialQuantity(result, uc, teeMaterial, "UND", Math.Abs(tees));
                }
            }
    ...
    private static void AddMaterialQuantity(Dictionary<UcKey, Dictionary<MaterialKey, double>> result, UcKey uc, MaterialSpec material, string unit, double quantity)
    {
        ...
        if (!result.TryGetValue(uc, out materials))
        {
            materials = new Dictionary<MaterialKey, double>();
            result.Add(uc, materials);
        }

        double current;
        materials.TryGetValue(materialKey, out current);
    ...
    private static bool TryGetMaterialSpec(string description, string diameter, out MaterialSpec material)
    {
        ...
        foreach (MaterialSpec candidate in MaterialCatalog)
        {
            ...
            if (!string.IsNullOrWhiteSpace(normalizedDiameter) && NormalizeDiameter(candidate.Diameter) == normalizedDiameter)
            {
                material = candidate;
                return true;
            }
        ...
        foreach (MaterialSpec candidate in MaterialCatalog)
        {
            ...
            if (normalizedDescription.IndexOf(candidateDiameter, StringComparison.OrdinalIgnoreCase) >= 0)
            {
                material = candidate;
                return true;
            }
    ...
    private static List<SourceMaterialRow> FindSourceMaterialsForActivity(ZipArchive archive, XElement workbook, XElement workbookRels, XNamespace mainNs, XNamespace relNs, XNamespace packageRelNs, string activity)
    {
        ...
        XElement sourceSheet = sheets.Elements(mainNs + "sheet").FirstOrDefault(x => string.Equals((string)x.Attribute("name"), "materiales leg", StringComparison.OrdinalIgnoreCase));
        if (sourceSheet == null) return result;
        ...
        if (sourceRel == null) return result;
        ...
        foreach (XElement row in sheetData.Elements(mainNs + "row").OrderBy(x => (int?)x.Attribute("r") ?? 0))
        {
            ...
            double numericCode;
            if (double.TryParse(codeText.Trim(), NumberStyles.Any, CultureInfo.InvariantCulture, out numericCode))
            {
                activityCode = Convert.ToInt32(numericCode, CultureInfo.InvariantCulture);
                break;
            }
        }
    ...
    {
        matchedKey = default(MaterialKey);
        quantity = 0.0;
    ...
    private static string NormalizeDiameter(string value)
    {
        ...
        normalized = normalized.Replace("PULG", string.Empty).Replace("PULGADAS", string.Empty).Replace(" ", string.Empty);
        return normalized;
    }
    ...
    private static string NormalizeSurface(string value)
    {
        ...
        foreach (UcSurface surface in Surfaces)
            if (NormalizeToken(surface.Name) == normalized)
                return surface.Name;
        return value.Trim();
    }
    ...
    private static Dictionary<int, string> LoadSharedStrings(ZipArchive archive, XNamespace mainNs)
    {
        ...
        foreach (XElement si in root.Elements(mainNs + "si"))
            result[i++] = string.Concat(si.Descendants(mainNs + "t").Select(x => x.Value));
        return result;
    }
    ...
    private static string ReadCellText(XElement cell, XNamespace mainNs, Dictionary<int, string> sharedStrings)
    {
        ...
        {
            int index;
            return int.TryParse(value.Value, NumberStyles.Integer, CultureInfo.InvariantCulture, out index) && sharedStrings.TryGetValue(index, out string text)
                ? text
                : string.Empty;
        }
    ...
    private static void RemoveCalculationChain(ZipArchive archive, XElement workbookRels, XNamespace packageRelNs)
    {
        ...
        foreach (XElement rel in workbookRels.Elements(packageRelNs + "Relationship").Where(x =>
            string.Equals((string)x.Attribute("Type"), "http://schemas.openxmlformats.org/officeDocument/2006/relationships/calcChain", StringComparison.OrdinalIgnoreCase) ||
            string.Equals((string)x.Attribute("Target"), "calcChain.xml", StringComparison.OrdinalIgnoreCase)).ToList())
            rel.Remove();
        ...
        if (contentEntry != null)
        {
            ...
            foreach (XElement item in content.Elements(ns + "Override").Where(x => string.Equals((string)x.Attribute("PartName"), "/xl/calcChain.xml", StringComparison.OrdinalIgnoreCase)).ToList())
                item.Remove();
    ...
    private struct UcKey : IEquatable<UcKey>
    {
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                return (StringComparer.OrdinalIgnoreCase.GetHashCode(Diameter ?? string.Empty) * 397) ^ StringComparer.OrdinalIgnoreCase.GetHashCode(Surface ?? string.Empty);
            }
        }
    }
    ...
    private struct MaterialKey : IEquatable<MaterialKey>
    {
        ...
        {
            Description = NormalizeToken(description);
            Diameter = NormalizeDiameter(diameter);
        ...
        public bool Equals(MaterialKey other) =>
            ...
            string.Equals(Diameter, other.Diameter, StringComparison.OrdinalIgnoreCase) &&
        ...
        public override int GetHashCode()
        {
            unchecked
            {
                int hash = StringComparer.OrdinalIgnoreCase.GetHashCode(Description ?? string.Empty);
                hash = (hash * 397) ^ StringComparer.OrdinalIgnoreCase.GetHashCode(Diameter ?? string.Empty);
                hash = (hash * 397) ^ StringComparer.OrdinalIgnoreCase.GetHashCode(Unit ?? string.Empty);
                return (hash * 397) ^ StringComparer.OrdinalIgnoreCase.GetHashCode(Code ?? string.Empty);
            }
        }
    }
    ...
    {
        public MaterialSpec(string description, string diameter, string code)
        {
    ...
    {
        public SourceMaterialRow(string code, string description)
        {
            Code = code;
            Description = description;
        }
