...
public sealed class ResumenUCTool
{
    ...
    private static string? GetSurface(Transaction transaction, Dimension dimension)
    {
        ...
        {
            if (surface.ColorIndex.HasValue && color.ColorIndex == surface.ColorIndex.Value)
                return surface.Name;

            if (surface.Red.HasValue && IsSameRgb(color, surface.Red.Value, surface.Green!.Value, surface.Blue!.Value))
                return surface.Name;
        }
    ...
    private static void CreateTexts(Database database, Point3d topLeftPoint, IReadOnlyDictionary<UcKey, double> quantities)
    {
        ...
        IEnumerable<KeyValuePair<UcKey, double>> orderedItems = quantities
            .OrderBy(x => x.Key.Diameter, StringComparer.OrdinalIgnoreCase)
        ...
        foreach (var item in orderedItems)
        {
            ...
            AddCenteredText(transaction, currentSpace, "ML",
                new Point3d(unitX, y, 0), TextHeight, layerName, textStyleId);
            AddCenteredText(transaction, currentSpace, FormatQuantity(item.Value),
                new Point3d(subtotalX, y, 0), TextHeight, layerName, textStyleId);
    ...
    private static string ToDisplaySurface(string value)
    {
